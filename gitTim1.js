...
function greedy_shortest(graph, source, dest) {
  //parameter tdd pilihan rute, lokasi berangkat dan lokasi tujuan
  result = [];
  result.push(source);
  var current_node;
  ...
  while (dest !== result[result.length - 1]) {
    ...
    var [localmax] = Object.entries(graph[current_node]).sort(
      ([, v1], [, v2]) => v1 - v2
    );
